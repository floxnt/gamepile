# Vendored pywebview Windows backend with .NET 8 (coreclr) compatibility patch.
#
# Provenance:
#   Base:    pywebview 6.2.1, webview/platforms/winforms.py
#            (https://github.com/r0x0r/pywebview tag 6.2.1)
#   Patch:   smparkes/pywebview fork, branch fix/dotnet8-coreclr,
#            commit a2bf0df4a4728b170cb02f1f1f698387fbaf0379
#            (https://github.com/smparkes/pywebview/commits/a2bf0df4a4728b170cb02f1f1f698387fbaf0379)
#   Context: pywebview issue #1803
#            (https://github.com/r0x0r/pywebview/issues/1803)
#
# Why this file exists:
#   pywebview 6.2.1's winforms.py crashes at module-import-time on .NET 8
#   coreclr because OpenFolderDialog's class body reflects over .NET-Framework-
#   internal types (System.Windows.Forms.FileDialogNative+IFileDialog,
#   System.Windows.Forms.FileDialogNative+FOS) that don't exist in .NET 8's
#   WinForms (Microsoft.WindowsDesktop.App). The smparkes patch splits
#   OpenFolderDialog into netfx and coreclr variants (the coreclr variant uses
#   the public WinForms.FolderBrowserDialog) and adds two defensive fixes for
#   .NET 8 environment variance (AddReference Microsoft.Win32.SystemEvents on
#   coreclr; guard the .NET Framework registry probe in edge_build()).
#
#   v0.5.3..v0.5.9 of GamePile shipped Windows bundles that did not launch on
#   clean consumer Windows 11 machines for a cascading sequence of related
#   reasons; v0.6.0 ships with this vendored patched winforms.py in place of
#   pywebview's, and is the first Windows-launching release.
#
# Lifecycle:
#   The release workflow's "Apply pywebview winforms.py patch" step
#   SHA512-verifies this file, copies it over the .venv's
#   webview/platforms/winforms.py after `uv sync` and before PyInstaller, then
#   SHA512-verifies the destination post-copy. See
#   SPEC_V5_DISTRIBUTION.md section "pywebview winforms.py patch".
#
# When to delete:
#   When pywebview ships a release (>6.2.1) that incorporates issue #1803's
#   upstream-aware fix. At that point: remove vendor/pywebview/, remove the
#   workflow's "Apply pywebview winforms.py patch" step, remove the SPEC
#   section, bump pywebview pin in pyproject.toml.
#
# Do NOT edit this file directly. If smparkes' patch needs updating, refresh
# the entire file from a newer upstream commit and update this header's
# pinned SHA. Keep the file 1:1 with upstream provenance so the diff against
# pywebview 6.2.1 stays auditable.
