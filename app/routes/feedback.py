...
def _pick(conn, pick_id):
    ...
    if pick.legacy_taste_recorded:
        raise HTTPException(
            409,
            "This older feedback is preserved in your existing taste history. Set a personal rating from the game details to give new feedback.",
        )
    return pick
...
def _complete(conn, pick_id):
    conn.execute(
        "UPDATE pick_history SET feedback_completed_at=? WHERE id=?",
        (datetime.utcnow().isoformat(), pick_id),
    )
...
def _candidates(conn, pick):
    ...
    return [
        games[appid]
        for appid in ids
        if appid in games and games[appid].state.status != GameStatus.not_interested
    ]
...
def resume_context(pick):
    ...
    if pick.outcome == "did_not_play" and pick.did_not_play_reason in (
        "changed_mood",
        "picked_another_game",
    ):
...
async def feedback_step1(request: Request, pick_id: int, answer: str = Form(...)):
    ...
    with db.get_db() as conn:
        ...
        pick = _pick(conn, pick_id)
        conn.execute(
            "UPDATE pick_history SET feedback_completed_at=NULL WHERE id=?", (pick_id,)
        )
        if answer == "yes" and pick.outcome == "did_not_play":
            conn.execute(
                "UPDATE pick_history SET did_not_play_reason=NULL, actually_played_appid=NULL, would_have_picked_other_appid=NULL WHERE id=?",
                (pick_id,),
            )
    return templates.TemplateResponse(
        request,
        (
            "partials/feedback_step1_5.html"
            if answer == "no"
            else "partials/feedback_step2.html"
        ),
        {"pick": pick},
    )
...
async def feedback_step1_5(request: Request, pick_id: int, reason: str = Form(...)):
    ...
    with db.get_db() as conn:
        ...
        pick = _pick(conn, pick_id)
        conn.execute(
            "UPDATE pick_history SET rating=NULL,genre_match_rating=NULL,actually_played_appid=NULL,would_have_picked_other_appid=NULL,feedback_completed_at=NULL WHERE id=?",
            (pick_id,),
        )
        db.update_pick_outcome(
            conn, pick_id, outcome="did_not_play", did_not_play_reason=reason
        )
        ...
        pick = _pick(conn, pick_id)
    return templates.TemplateResponse(
        request, "partials/feedback_step1_6.html", {"pick": pick}
    )
...
async def feedback_step1_6(request: Request, pick_id: int, sub_option: str = Form(...)):
    ...
    name = (
        "partials/feedback_step1_6_candidates.html"
        if sub_option == "from_candidates"
        else "partials/feedback_step1_6_library.html"
    )
    return templates.TemplateResponse(
        request, name, {"pick": pick, "other_candidates": candidates}
    )


@router.post("/feedback/{pick_id}/step1_6_candidates")
async def feedback_step1_6_candidates(
    pick_id: int, selected_appid: Optional[int] = Form(None)
):
    with db.get_db() as conn:
        taste.write_lock(conn)
        pick = _pick(conn, pick_id)
        if selected_appid and (
            selected_appid == pick.appid
            or selected_appid not in _parse_candidate_appids(pick.candidates_at_pick)
        ):
            raise HTTPException(400, "Game was not one of these recommendations")
        _game(conn, selected_appid)
        conn.execute(
            "UPDATE pick_history SET would_have_picked_other_appid=? WHERE id=?",
            (selected_appid, pick_id),
        )
...
async def feedback_game_search(request: Request, pick_id: int, q: str = ""):
    ...
    return templates.TemplateResponse(
        request,
        "partials/feedback_game_search_results.html",
        {"results": results, "pick_id": pick_id},
    )


@router.post("/feedback/{pick_id}/step1_6_library")
async def feedback_step1_6_library(
    pick_id: int, actually_played_appid: Optional[int] = Form(None)
):
    with db.get_db() as conn:
        taste.write_lock(conn)
        pick = _pick(conn, pick_id)
        ...
        conn.execute(
            "UPDATE pick_history SET actually_played_appid=? WHERE id=?",
            (actually_played_appid, pick_id),
        )
...
async def feedback_step2(
    ...
    with db.get_db() as conn:
        ...
        conn.execute(
            "UPDATE pick_history SET rating=?,feedback_completed_at=NULL WHERE id=?",
            (None if skip else rating, pick_id),
        )
        db.update_pick_outcome(
            conn,
            pick_id,
            outcome=(
                _rating_to_outcome(rating, status) if not skip else "played_still_going"
            ),
        )
        _save_played(conn, pick_id)
        pick = _pick(conn, pick_id)
    return templates.TemplateResponse(
        request, "partials/feedback_step3.html", {"pick": pick}
    )
...
async def feedback_step3(
    ...
    with db.get_db() as conn:
        ...
        _pick(conn, pick_id)
        conn.execute(
            "UPDATE pick_history SET genre_match_rating=? WHERE id=?",
            (None if skip else genre_match, pick_id),
        )
        _save_played(conn, pick_id)
        pick = _pick(conn, pick_id)
        candidates = _candidates(conn, pick)
    return templates.TemplateResponse(
        request,
        "partials/feedback_step4.html",
        {"pick": pick, "other_candidates": candidates},
    )


@router.post("/feedback/{pick_id}/step4")
async def feedback_step4(
    pick_id: int, answer: str = Form(...), retroactive_appid: Optional[int] = Form(None)
):
    ...
    with db.get_db() as conn:
        ...
        if answer == "yes" and (
            not other_id
            or other_id == pick.appid
            or other_id not in _parse_candidate_appids(pick.candidates_at_pick)
        ):
