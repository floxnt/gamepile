...
MESSAGES = {
    ...
    "already_completed": "Marked finished",
    "confirm_finished": "Marked finished",
    "mark_in_progress": "Marked in progress",
    "pick": "Marked in progress",
    "bounced": "Marked bounced",
    "not_my_thing": "Marked not my thing",
    "never_recommend": "Excluded from recommendations",
    "restore": "Restored to recommendations",
    "pin": "Pinned to Shortlist",
    "unpin": "Unpinned",
    "clear_technical_issue": "Technical issue cleared",
}
...
def _snapshot(conn, appid, pick_id=None):
    ...
    state = dict(
        conn.execute("SELECT * FROM game_state WHERE appid=?", (appid,)).fetchone()
    )
    row = conn.execute(
        "SELECT * FROM taste_signals WHERE source=?", (f"quick:{appid}",)
    ).fetchone()
    pick = (
        conn.execute("SELECT * FROM pick_history WHERE id=?", (pick_id,)).fetchone()
        if pick_id
        else None
    )
    return {
...
def record_undo(conn, appid, before, after):
    ...
    conn.execute(
        "DELETE FROM action_undo WHERE created_at < ?",
        ((datetime.utcnow() - timedelta(days=1)).isoformat(),),
    )
    conn.execute(
        "INSERT INTO action_undo VALUES (?,?,?,?,?)",
        (
            token,
            appid,
            json.dumps(before),
...
def perform(conn, appid, action, pick_id=None):
    ...
    if action in ("finished", "already_completed", "confirm_finished"):
        ...
        db.update_game_state(
            conn, appid, status=GameStatus.in_progress, manually_set=True
        )
        db.clear_pin(conn, appid)
        taste.set_signal(conn, f"quick:{appid}", appid, [])
        ...
        db.update_game_state(
            ...
            dropped_strength="strong" if strong else "soft",
            manually_set=True,
        )
        db.clear_pin(conn, appid)
        taste.set_signal(
            conn, f"quick:{appid}", appid, taste.labels(game, -1 if strong else -0.5)
        )
        if pick_id:
        ...
        if state.status == GameStatus.not_interested or (
            state.status == GameStatus.dropped and state.dropped_strength == "strong"
        ):
...
def undo(conn, token):
    ...
    if current != after:
        raise HTTPException(
            409, "This game has changed since that action. Open its details to edit it."
        )
    state = before["state"]
    ...
    taste.set_signal(
        ...
        f"quick:{appid}",
        appid,
        json.loads(signal["contributions"]) if signal else [],
    )
    if signal:
        conn.execute(
            "UPDATE taste_signals SET updated_at=? WHERE source=?",
            (signal["updated_at"], signal["source"]),
        )
    if before["pick_id"]:
        if before["pick"]:
            pick = before["pick"]
            columns = [k for k in pick if k != "id"]
            conn.execute(
            ...
            conn.execute(
                "DELETE FROM taste_signals WHERE source=?",
                (f"pick:{before['pick_id']}",),
            )
