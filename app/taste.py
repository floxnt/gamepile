...
def set_signal(conn, source: str, appid: int, contributions: list[dict]):
    write_lock(conn)
    if contributions:
        conn.execute("""INSERT INTO taste_signals(source,appid,contributions,updated_at)
...
def labels(game, delta: float, *, confidence=True):
    ...
    return [{"kind":kind,"value":value,"delta":delta,"confidence":confidence}
        for kind,value in deduplicate_labels(game.genre_list(),game.user_tags_list(),game.developer)]


def set_personal_rating(conn, game, rating):
    # Same endpoints as session ratings: 1★=-1, 3★=0, 5★=+1.
    values = [] if rating is None else labels(game, max(-1, min(1, (rating / 2 - 3) / 2)))
...
def rebuild(conn):
    ...
    conn.executemany("INSERT INTO affinity(kind,value,weight,pick_count,updated_at) VALUES (?,?,?,?,?)",
